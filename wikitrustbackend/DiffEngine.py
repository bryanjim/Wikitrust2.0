...
def quality(l, i1, len1, i2, len2):
    ...
    return (1.0 * l) / min(len1, len2) - 0.3 * abs(
        i1 / (1.0 * len1) - i2 / (1.0 * len2)
    )
    ...
    while matches != []:
        ...
        if not (matched1[i1] or matched2[i2]):
            ...
            if not (matched1[i1 + l - 1] or matched2[i2 + l - 1]):
                ...
                while matched1[i1 + j] or matched2[i2 + j]:
                    j -= 1
                l = j + 1
            ...
            if not (matched1[i1 + l - 1] or matched2[i2 + l - 1]):
                ...
                if (
                    matched1[i1] != matched1[i1 + l - 1]
                    and matched2[i2] != matched2[i2 + l - 1]
                ):
                    j = 1
                    while j < l - 1 and (matched1[i1 + j] or matched2[i2 + j]):
                        j += 1
                    if j < l - 1:
                        # Beginning at j, there is some portion that is not matched yet.
                        k = j + 1
                        while k < l - 1 and not (matched1[i1 + k] or matched2[i2 + k]):
                            k += 1
...
def edit_diff_tichy(w1, w2, idx2):
    ...
    if len2 > first_unmatched_2:
        ...
        diff.append(
            (DELETE, first_unmatched_2, first_unmatched_2, len2 - first_unmatched_2)
        )
...
class TestDiffsGreedy(unittest.TestCase):
    ...
    def test0(self):
        print()
        print("=========================================")
        s1 = "Hi this is joseph jumping over the fence"
        s2 = "jumping fence over this is joseph"
        r = test_greedy(s1, s2)
        print(s1)
        print(s2)
        print_edit_diff(r)
...
class TestDiffsTichy(unittest.TestCase):
    ...
    def test6(self):
        ...
        self.assertCountEqual(
            [(0, 0, 0, 4), (2, 4, 4, 10), (0, 14, 4, 4)], [x] + w + [y]
        )
...
# ############################################################### - Cagan Bakirci
# #Read the two files to compare. For now, assume the files
# #are in the directory. See if the files will be opened.
# f = open("test1.txt")
# t = open("test2.txt")
# firstFile = f.read()
# secondFile = t.read()
...
# greedyDiffVal = test_greedy(firstFile, secondFile)
# tichyDiffVal = test_tichy(firstFile, secondFile)

# print("---The Greedy Output---")
# print (greedyDiffVal)
# print()

# print("Expected Greedy Output")
# print([(0, 3, 2, 4), (0, 11, 13, 3), (0, 17, 18, 3),
#                               (0, 0, 9, 3), (0, 8, 7, 2), (2, 7, 7, 1), (2, 10, 10, 1),
#                               (2, 14, 14, 3), (1, 0, 0, 2), (1, 6, 6, 1), (1, 12, 12, 1),
#                               (1, 16, 16, 2), (1, 21, 21, 9)])

# print("---The Tichy Output---")
# print (tichyDiffVal)
# print()

# print("Expected Tichy Output")
# print([(0, 0, 9, 3), (0, 3, 2, 4), (2, 7, 7, 1), (0, 8, 7, 2), (2, 10, 10, 1),
#                               (0, 11, 13, 3), (2, 14, 14, 3), (0, 17, 18, 3), (1, 0, 0, 2), (1, 6, 6, 1),
#                               (1, 12, 12, 1), (1, 16, 16, 2), (1, 21, 21, 9)])

# #Open a file to write the output
# k = open("diff.txt", 'w')
# k.write(str(greedyDiffVal))
# k.close()
